...
function buscarPorIndice(req, res) { //req, res são os parâmetros que representam a requisição e a resposta do Express.js. O parâmetro req contém informações sobre a requisição HTTP, como parâmetros de rota, corpo da requisição, cabeçalhos, etc. O parâmetro res é usado para enviar a resposta de volta ao cliente.
  ...
  if (!livro) { // Se o livro não for encontrado (ou seja, se a função buscarLivroPorIndice retornar null ou undefined), o código dentro do bloco if será executado. 
